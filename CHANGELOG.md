...
- **Comment button flashed between clicks of a double/triple-click**: showing the action button is now debounced (250ms, resets on each `mouseup`), so it appears only once the selection settles instead of popping up mid multi-click. The debounce also lets the webview finalize the selection before it's read. Dismissing an open comment bubble by clicking outside it stays immediate (not debounced).
- **Sidebar still went stale when files were added/removed externally**: the file-event handlers from #15 (`onDidCreateFiles`/`onDidDeleteFiles`/`onDidRenameFiles`) only fire for VS Code-mediated operations, so files created by the terminal, git, or other tools were missed. Replaced them with a `**/*.md` `FileSystemWatcher` (ignoring `node_modules`/`.git` churn), which also catches external changes.
